...
def paivita_hinta():
    ...
    if hinta is not None:
        ...
        # Vaihdetaan väri hintatason mukaan
        if hinta < 5:
            hinta_label.config(fg="green")
        elif 5 <= hinta <= 15:
            hinta_label.config(fg="orange")
        else:  # yli 15
            hinta_label.config(fg="red")
    else:
        hinta_label.config(text="Ei saatavilla", fg="black")
...
hinta_label = tk.Label(frame, text="Ladataan...", font=("Helvetica", 32))
